...
const TimeAndDate = () => Widget.EventBox({
    ...
    child: Widget.Box({
        ...
        children: [ 
            Widget.Label({
                className: 'bar-clk',
            }) .poll(20000, label => label.label = Utils.execAsync(['date', '+%H : %M'])),
            
            Widget.Revealer({
                transitionDuration: 600,
                transition: 'slide_right',
                connections: [[isDateShown, self => {
                    self.revealChild = isDateShown.getValue();
                }]],
                child: Widget.Label({
                    connections: [
                        // once per hour
                        [3600000, self => execAsync(['date', '+%A %d %B'])
