...
if [[ $1 == rc ]]; then
    grim -g "$(slurp -b '#000000b0' -c '#00000000')" - | wl-copy --type image/png
    notify-send 'Selezione Copiata!' 

elif [[ $1 == rf ]]; then
    filename=${XDG_SCREENSHOTS_DIR}/$(date +%Y-%m-%d_%H-%M-%S).png
    ...
    filename=${XDG_SSCREENSHOTS_DIR}/%Y-%m-%d_%H-%M-%S.png
    grim - | wl-copy --type image/png
    notify-send 'Schermata Copiata!'

elif [[ $1 == sf ]]; then
    filename=${XDG_SSCREENSHOTS_DIR}/$(date +%Y-%m-%d_%H-%M-%S).png
    grim $filename
